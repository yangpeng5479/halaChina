...
- (void)buildFooterView {
    ...
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width - 80, self.pageControl.frame.origin.y+20, 80, self.pageControl.frame.size.height)];
    ...
    [self.skipButton setTitle:NSLocalizedString(@"Skip", @"跳过") forState:UIControlStateNormal];
    [self.skipButton addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.skipButton];
    
    startBut = [ACPButton buttonWithType:UIButtonTypeCustom];
    startBut.frame = CGRectMake(10, self.frame.size.height - self.pageControlY+20, self.frame.size.width-20, 35);
    [startBut setStyleRedButton];
    [startBut setTitle: NSLocalizedString(@"Start", @"开始") forState:UIControlStateNormal];
    [startBut addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.currentPageIndex == _pages.count - 1)
    {
        startBut.hidden = NO;
