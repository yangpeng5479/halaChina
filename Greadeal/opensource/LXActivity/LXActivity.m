...
#define SHAREBUTTONTITLE_FONT                   MOLightFont(14)

#define CANCEL_BUTTON_COLOR                     MOAppTextBackColor()

#define SHAREBUTTON_WIDTH                       50
#define SHAREBUTTON_HEIGHT                      50
#define SHAREBUTTON_INTERVAL_WIDTH   (([[UIScreen mainScreen] bounds].size.width-SHAREBUTTON_WIDTH*3)/4)
...
#define BUTTON_INTERVAL_WIDTH                   (([[UIScreen mainScreen] bounds].size.width-BUTTON_WIDTH)/2)

#define BUTTONTITLE_FONT                        MOBlodFont(18)
...
- (id)initWithTitle:(NSString *)title delegate:(id<LXActivityDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle ShareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray;
{
    self = [super init];
    if (self) {
        iconArrars = [[NSArray alloc] initWithArray:shareButtonImagesNameArray];
...
//    UILabel *shareLabel = [[UILabel alloc] initWithFrame:CGRectMake(SHARETITLE_INTERVAL_WIDTH+((line-1)*(SHARETITLE_INTERVAL_WIDTH+SHARETITLE_WIDTH)), SHARETITLE_INTERVAL_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)), SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
    ...
    UILabel *shareLabel = [[UILabel alloc] initWithFrame:CGRectMake(SHARETITLE_INTERVAL_WIDTH+((line-1)*(SHARETITLE_INTERVAL_WIDTH+SHARETITLE_WIDTH)-15), SHARETITLE_INTERVAL_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)), SHARETITLE_WIDTH+30, SHARETITLE_HEIGHT)];
    ...
    MODebugLayer(shareLabel, 1.f, [UIColor redColor].CGColor);
    ...
    if (self.delegate) {
        if (button.tag != self.postionIndexNumber-1) {
            if ([self.delegate respondsToSelector:@selector(didClickOnImageIndex:)] == YES) {
                ...
                NSString*  stricon = [iconArrars objectAtIndex:(int)button.tag];
                
                [self.delegate didClickOnImageIndex:stricon];
            }
        }
        else{
