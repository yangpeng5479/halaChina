...
#define kTextSelectColor [UIColor colorWithRed:(231/255.0) green:(23/255.0) blue:(72/255.0) alpha:1.0]
...
- (void)selectIndexPath:(DOPIndexPath *)indexPath triggerDelegate:(BOOL)trigger {
    ...
    if (indexPath.item < 0 ) {
        ...
        self.currentSelectText = title.string;
        if (trigger) {
            [_delegate menu:self didSelectRowAtIndexPath:indexPath];
        }
        if (_currentSelectRowArray.count > indexPath.column) {
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        if ([GDPublicManager instance].screenWidth>320)
            _fontSize = 14;
        else
            _fontSize = 13;
...
- (CGSize)calculateTitleSizeWithString:(NSString *)string
{
    ...
    NSDictionary *dic = @{NSFontAttributeName: MOLightFont(_fontSize)};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _leftTableView) {
        ...
        if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:_currentSelectedMenudIndex]<=0)
        {
            cell.accessoryView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@""] highlightedImage:[UIImage imageNamed:@"tick.png"]];
        } else {
            cell.accessoryView = nil;
        }
    ...
    cell.textLabel.font = MOLightFont(_fontSize);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        self.currentSelectText = [self confiMenuWithSelectItem:indexPath.item];
...
- (NSString*)confiMenuWithSelectItem:(NSInteger)item {
