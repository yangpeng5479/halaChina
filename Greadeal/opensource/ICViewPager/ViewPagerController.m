...
#define kDefaultTabHeight 45.0 // Default tab height
#define kDefaultTabOffset 56.0 // Offset of the second and further tabs' from left
#define kDefaultTabWidth 80.0

#define kDefaultTabLocation 1.0 // 1.0: Top, 0.0: Bottom

#define kDefaultStartFromLastTab 0.0 // 1.0: YES, 0.0: NO
...
- (IBAction)handleTapGesture:(id)sender {
    ...
    //LOG(@"%@",weakPageViewController.view);
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    {
        for (UILabel *view in tabView.subviews)
        {
            if ([view isKindOfClass:[UILabel class]])
            {
                if (tabView.selected)
                    view.textColor = MOAppTextBackColor();
                else
                    view.textColor = MOColor66Color();
            }
        }
    }
...
- (void)defaultSettings {
    ...
    _startFromLastTab = kDefaultStartFromLastTab;
...
//        
        ...
        _startFromLastTab = [self.delegate viewPager:self valueForOption:ViewPagerOptionLastFromSecondTab withDefault:kDefaultStartFromLastTab];
    ...
    for (int i = 0; i < _tabCount; i++) {
        ...
        //LOG(@"tabView.width=%ld",tabView.tag);
        CGRect frame = tabView.frame;
        frame.origin.x = contentSizeWidth;
        
        frame.size.width = tabView.frame.size.width;//self.tabWidth;
        tabView.frame = frame;
    ...
    if (self.startFromLastTab) {
        viewController = [self viewControllerAtIndex:_tabCount-1];
    } else {
    ...
    // Set activeTabIndex
    if (self.startFromLastTab)
        self.activeTabIndex = _tabCount-1;
    else
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[_tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        float width = tabViewContent.tag;
        tabViewContent.tag = index;
        // Create TabView and subview the content
        TabView *tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, width, self.tabHeight)];
