//
//  GDShopDeliveryOrderViewController.m
//  Greadeal
//
...
@interface GDShopDeliveryOrderViewController ()

@end

@implementation GDShopDeliveryOrderViewController
@synthesize  vendorId;
...
- (void)completePay
{
    [UIAlertView showWithTitle:NSLocalizedString(@"Order Successful", @"购买成功")
                       message:NSLocalizedString(@"Please waiting for the delivery", @"请等待送货")
                      ...
                      tapBlock:^(UIAlertView *alertView, NSInteger buttonIndex) {
                          if (buttonIndex == [alertView cancelButtonIndex]) {
                              ...
                              [CATransaction setCompletionBlock:^{
                                  GDShopDeliveryOrderDetailsViewController* nv = [[GDShopDeliveryOrderDetailsViewController alloc] initWithOrderId:order_id];
...
- (void)tapMakeOrder
{
    ...
    NSString* url = [NSString stringWithFormat:@"%@%@",[GDPublicManager instance].APIBaseUrl,@"rest2/v1/SaleOrder/gen_order"];
    ...
    parameters = @{@"product_list_json":jsonStr,@"token":[GDPublicManager instance].token,@"language_id":@([[GDSettingManager instance] language_id:NO]),@"comment":comment,@"vendor_id":@(vendorId),@"payment_code":choosePayType,@"address_id":@(sel_address_id)};
...
- (void)getPayWay
{
    ...
    parameters = @{@"type":@"sale",@"language_id":@([[GDSettingManager instance] language_id:NO])};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if (indexPath.section == kPriceSection)
    {
        ...
        ordersPrice.text = [NSString stringWithFormat:NSLocalizedString(@"Sub Total: %@%d",@"小计: %@%d"),[GDPublicManager instance].currency, sprice];
...
//            payPrice.text = [NSString stringWithFormat:NSLocalizedString(@"Cash/Card On Delivery Charges: %@%d",@"货到付款手续费: %@%d"),[GDPublicManager instance].currency, chosseCodFee];
...
//        
        ...
        sumPrice.text = [NSString stringWithFormat:NSLocalizedString(@"Total: %@%d",@"总金额: %@%d"),[GDPublicManager instance].currency, sprice+chosseCodFee+deliveryFee];
