...
@interface GDLiveDiscountViewController ()

@end

@implementation GDLiveDiscountViewController
...
- (void)getAllCategory
{
    categoryType selType;
    if (justDiscount)
         selType = CATEGORY_DISCOUNT_STORE;
    else
         selType = CATEGORY_ALL_STORE;
    
    [[GDPublicManager instance] getCategory:selType success:^(NSError *error)
    {
        [ProgressHUD dismiss];
        if (error!=nil)
        {
        ...
        else
        {
            ...
            if (selType == CATEGORY_DISCOUNT_STORE)
                categoryArrays = [[GDSettingManager instance].nDiscountStoreCategory mutableCopy];
            else
                categoryArrays = [[GDSettingManager instance].nAllStoreCategory mutableCopy];
...
- (void)getVendorData:(int)cid
{
    ...
    if (justDiscount)
    {
        parameters = @{@"language_id":@([[GDSettingManager instance] language_id:NO]),@"category_id":@(cid),@"page":@(seekPage),@"limit":@(prePageNumber),@"sort":sortChoose,@"country_id":@([GDSettingManager instance].currentCountryId),@"longitude":@([[GDSettingManager instance] getCityLongitude]),@"latitude":@([[GDSettingManager instance] getCityLatitude]),@"on_sale":@(1)};
    }
    else
    {
         parameters = @{@"language_id":@([[GDSettingManager instance] language_id:NO]),@"category_id":@(cid),@"page":@(seekPage),@"limit":@(prePageNumber),@"sort":sortChoose,@"country_id":@([GDSettingManager instance].currentCountryId),@"longitude":@([[GDSettingManager instance] getCityLongitude]),@"latitude":@([[GDSettingManager instance] getCityLatitude])};
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0)
    {
        ...
        {
            GDLiveImageVendorListCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
            if (!cell) {
                cell = [[GDLiveImageVendorListCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:CellIdentifier];
            }
            [cell.productImage sd_setImageWithURL:[NSURL URLWithString:[vendorImageUrl encodeUTF]]
                                 placeholderImage:[UIImage imageNamed:@"live_product_default.png"]];
