//
//  GDDeliveryMenuViewController.m
//  Greadeal
//
...
#import "GDDeliveryMenuViewController.h"
...
@interface GDDeliveryMenuViewController ()

@end

@implementation GDDeliveryMenuViewController
...
- (id)init:(NSDictionary*)vender_info
{
    ...
    if (self)
    {
        ...
        SET_IF_NOT_NULL(openTimeArray,vender_info[@"open_time"]);
        if (openTimeArray.count>0)
        {
            ...
            if (dict!=nil)
            {
                ...
                sTime = [NSString stringWithFormat:@"%@",[open_time_start substringToIndex:5]];
                eTime = [NSString stringWithFormat:@"%@",[open_time_end substringToIndex:5]];
            }
        }
...
{
    BOOL isopen = [self checkStoreOpenTime];
    if (!isopen){
        [UIAlertView showWithTitle:nil
                           message:NSLocalizedString(@"Oops! The store is closed now!", @"哎呀！目前商店已经休息!")
    ...
    if ([GDPublicManager instance].cid>0)
    {
        ...
        GDDeliveryOrderViewController* vc = [[GDDeliveryOrderViewController alloc] init:orderArrar withDeliveryFee:deliverFee withDiscount:discount];
    ...
    for (int line = 0; line <leftData.count; line++)
    {
        ...
        if (temp.count>0)
        {
            for (NSDictionary* obj in temp)
            {
                ...
                sprice += [obj[@"price"] intValue]*order_qty;
            }
        }
    }
