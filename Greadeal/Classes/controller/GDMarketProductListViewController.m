//
//  GDMarketProductListViewController.m
//  Greadeal
//
...
@interface GDMarketProductListViewController ()

@end

@implementation GDMarketProductListViewController
...
- (void)getProductData
{
    ...
    url = [NSString stringWithFormat:@"%@%@",[GDPublicManager instance].APIBaseUrl,@"supermarket/get_product_list_of_vendor_by_category_id"];
    parameters = @{@"language_id":@([[GDSettingManager instance] language_id:YES]),@"vendor_id":@(vendorId),@"category_id":@(categoryId),@"page":@(seekPage),@"limit":@(prePageNumber)};
